/* ============================================================
   printerItem.js
   Logic for the Printer Detail / Edit page (printerItem.html)
   ============================================================ */
...
(() => {
    ...
    function canEditGeneral() {
        const has = getUserPermissions().includes(26);
        console.log("[Permission 26 — Edit Asset: Printer]", has ? "GRANTED" : "DENIED", "| Permissions:", getUserPermissions());
        return has;
    }
    function canEditNetwork() {
        const perms = getUserPermissions();
        const has = perms.includes(30);
        console.log("[Permission 30 — Edit Asset: Printer (Network Fields)]", has ? "GRANTED" : "DENIED", "| Permissions:", perms);
        return has;
    }
    ...
    const printerId = getParam("id");

    // Track whether the restricted Network & Connectivity section is in edit mode
    const sectionEditState = { network: false };
    ...
    const sectionEditSnapshots = { network: null };
    ...
    async function loadAsset() {
        if (!printerId) {
            renderError("No printer ID specified in the URL.");
            return;
        }

        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`/api/printer/${printerId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            console.error(err);
            renderError("Failed to load printer. Please check the ID or try again.");
        }
    }

    // ── Render Page ──────────────────────────────────────────
    function renderPage(d) {
        const nameLabel = d.printer_name || `Printer #${d.printer_id}`;

        qs("#breadcrumbName").textContent = nameLabel;
        qs("#pageTitle").textContent      = nameLabel;
        document.title = `${nameLabel} — Printer Detail`;
    ...
    function buildBasicsCard(d) {
        ...
        const conditionOpts = ["New", "Good", "Fair", "Poor"];
        const brandFallback = ["Canon", "HP", "Epson", "Brother", "Xerox", "Ricoh", "Kyocera", "Lexmark", "Samsung", "Other"];
        // Optional field — include a real "None" value rather than a forced
        // "please select" placeholder, since a printer may not yet have a
        // location on record.
        const locationOpts = [{ value: "", label: "None" }, ...LOCATION_OPTS];

        return `
        <div class="section-card" id="card-basics">
            ${sectionHeader("card-basics", "printer", "Asset Basics", "Identification and classification")}
            <div class="section-body form-grid">
                ${fg("Printer Name",    val(d.printer_name),    inp("printer_name",    d.printer_name,    "text", "e.g. PRN-4F-001", false), false)}
                ${fg("Serial Number",   val(d.serial_no),        inp("serial_no",        d.serial_no,       "text", "",               true), true)}
                ${fg("Brand",           val(d.brand),            catSel("brand",         "printer.brand",   brandFallback, d.brand),          true)}
                ...
                ${fg("Asset Tag",       val(d.asset_tag),        inp("asset_tag",        d.asset_tag,       "text", "e.g. TMCSL-PRN-001"),      false, true)}
                ${fg("Asset Location",  val(d.asset_location),   sel("asset_location",   locationOpts,      d.asset_location),                  false)}
            </div>
        </div>`;
    }
    ...
    function buildSpecsCard(d) {
        ...
        const connectivityOpts     = ["", "LAN", "WIFI", "WIFI & LAN",];
        const isColorVal           = d.is_color === 1 || d.is_color === true;
        const isColorDisplay       = isColorVal ? "Yes — Color" : "No — Monochrome";

        const deployedVal  = toLocalDateInput(d.date_deployed);

        return `
        <div class="section-card" id="card-specs">
            ${sectionHeader("card-specs", "settings-2", "Printer Specifications", "Type, color capability, and department")}
            <div class="section-body form-grid">
                ${fg("Printer Type",    val(d.printer_type),    catSel("printer_type",  "printer.printer_type", printerTypeFallback, d.printer_type), true)}
                ${fg("Color Capable",   isColorDisplay,         sel("is_color", [
                    { value: "true",  label: "Yes — Color" },
                    { value: "false", label: "No — Monochrome" },
                ], String(isColorVal)),                                                                                                                 true)}
                ${fg("Department",      val(d.department_name), `<div class="dept-search-wrap" id="dept-search-wrap">
                    <input
                        type="text"
                        ...
                        autocomplete="off"
                        value="${escHtml(d.department_name || "")}"
                    />
                    <div class="dept-dropdown" id="dept-dropdown"></div>
                    <input type="hidden" id="department" value="${escHtml(iVal(d.department))}" />
                </div>`,                                                                                                                               true)}
                ${fg("Date Deployed",   fmtDate(d.date_deployed), inp("date_deployed",  deployedVal,    "date"), true)}
            </div>
        </div>`;
    }
    ...
    function buildNetworkCard(d) {
        const connectOpts = [
            ...
            { value: "LAN & WIFI", label: "WIFI & LAN" },
        ];
        const canEdit = canEditNetwork();

        return `
        <div class="section-card" id="section-card-network">
            ${sectionHeader("section-card-network", "network", "Network &amp; Connectivity", "IP address, MAC, and connection type", buildSectionEditControls("network", canEdit, "Edit Asset: Printer (Network Fields)"))}
            <div class="section-body form-grid">
                ${fgRestricted("network", "Connectivity",  val(d.connectivity), sel("connectivity", connectOpts, d.connectivity))}
                ${fgRestricted("network", "IP Address",    val(d.ip_address),   inp("ip_address",  d.ip_address,  "text", "e.g. 192.168.1.200"))}
                ${fgRestricted("network", "MAC Address",   val(d.mac_address),  inp("mac_address", d.mac_address, "text", "e.g. 11:22:33:44:55:66"), false, true)}
            </div>
        </div>`;
    }
    ...
    function buildProcurementCard(d) {
        ...
        <div class="section-card" id="card-procurement">
            ...
            <div class="section-body form-grid">
                ...
                ${fg("Remarks",           val(d.remarks), `<textarea id="remarks" placeholder="Any notes about this printer…">${escHtml(d.remarks || "")}</textarea>`, false, true)}
            </div>
        </div>`;
    }
    ...
    function buildQuickInfoBar(d) {
        ...
        const deptBlock = d.department_name
            ? `<div class="assigned-card quick-info-dept">
                   ...
                   <div class="user-info">
                       <div class="name">${escHtml(d.department_name)}</div>
                       <div class="meta">
                           ${d.asset_location ? `Location: ${escHtml(d.asset_location)}` : ""}
                           ${d.date_deployed
                               ? `${d.asset_location ? " · " : ""}Deployed ${fmtDate(d.date_deployed)}`
                               : ""}
                       </div>
                   </div>
               </div>`
            : `<div class="no-user quick-info-dept">No department assigned</div>`;
        ...
        <div class="section-card quick-info-bar">
            ...
            <div class="info-grid">
                ...
                <div class="info-cell"><span class="label">Printer Type</span>  <span class="value">${val(d.printer_type)}</span></div>
                <div class="info-cell"><span class="label">Color</span>         <span class="value">${isColorVal ? "✓ Color" : "✗ Monochrome"}</span></div>
                <div class="info-cell"><span class="label">Warranty</span>      <span class="value">${wLabel}</span></div>
                <div class="info-cell"><span class="label">Connectivity</span>  <span class="value">${val(d.connectivity)}</span></div>
    ...
    // user lacks permission 26, instead of alerting at click time.
    function applyGlobalEditButtonState() {
        ...
        if (canEditGeneral()) {
            ...
            if (!wrapper) {
                ...
                newWrapper.dataset.tooltip = "You need Edit Asset: Printer permission to edit this asset.";
    ...
    window.saveChanges = async function () {
        ...
        const requiredFields = [
            ...
            { id: "printer_type",   label: "Printer Type" },
            { id: "is_color",       label: "Color Capable" },
            { id: "department",     label: "Department" },
            { id: "date_deployed",  label: "Date Deployed" },
        ...
        requiredFields.forEach(({ id, label }) => {
            ...
            if (isEmpty) {
                ...
                if (id === "department") {
                    qs("#dept-search-input")?.classList.add("input-error");
                }
                valid = false;
                firstInvalidId = firstInvalidId || (id === "department" ? "dept-search-input" : id);
            } else {
                setFieldError(id, null);
                if (id === "department") {
                    qs("#dept-search-input")?.classList.remove("input-error");
                }
            }
        });
        ...
        // Map snake_case input IDs → camelCase API field names for PUT /printer/{id}
        const payload = {
            ...
            assetLocation:  raw.asset_location  || null,
            printerType:    raw.printer_type,
            isColor:        raw.is_color === "true",
            department:     parseInt(raw.department) || null,
            dateDeployed:   localDate(raw.date_deployed),
            connectivity:   assetData.connectivity  ?? null,
        ...
        try {
            ...
            // Resolve department_name from the selected department id
            const selectedDept = departments.find(dep => dep.department_id === parseInt(raw.department));

            // Merge changes back into local state (kept in snake_case internally)
            assetData = {
                ...assetData,
                ...raw,
                is_color:        raw.is_color === "true",
                department:      parseInt(raw.department) || assetData.department,
                department_name: selectedDept ? selectedDept.department_name : assetData.department_name,
            };
    ...
    window.cancelSectionEdit = async function (section) {
        ...
        if (card) {
            card.outerHTML = buildNetworkCard(assetData);
            lucide.createIcons();
        }
        applyRestrictedSectionStates();
    };
    ...
    window.saveSectionEdit = async function (section) {
        ...
        // ── Network: PATCH /printer/{id}/network ──────────────
        if (section === "network") {
            const payload = {
                connectivity: raw.connectivity || null,
                ipAddress:    raw.ip_address    || null,
                macAddress:   raw.mac_address   || null,
            };

            try {
                const res = await fetch(`/api/printer/${printerId}/network`, {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify(payload)
                });
    ...
    function initDeptSearch() {
        ...
        const hiddenInput = qs("#department");
            ...
            setTimeout(() => {
                ...
                if (!hiddenInput.value) {
                    ...
                    if (match) {
                        ...
                        hiddenInput.value = assetData.department      || "";
                    }
                }
            }, 150);
        });
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        await Promise.all([loadDepartments(), loadCategories(), loadVendors()]);
        loadAsset();
    });
