document.addEventListener("DOMContentLoaded", async () => {
        ...
        function isValidSelection() {
            ...
            if (hidden) {
                ...
                const match = options.find(
                    o => (typeof o === "object" ? o.label : o).toLowerCase() === val.toLowerCase()
                );
                if (match) {
    ...
    const LOCKED_EU_FIELD_IDS = ["euName", "euEmpId", "euDivision", "euDepartment", "euLocation", "euEmail", "euContactNumber"];
    ...
    function initEndUserSearchCombo() {
        ...
        function enforceSelection() {
            const ok = isValidSelection();
            setComboError(ok ? null : "Please select an employee from the dropdown list.");
            return ok;
        }
    ...
    function autofillEndUser(u) {
        ...
        document.getElementById("euContactNumber").value = u.eu_contact_number ?? u.eu_contact_no ?? "";
    ...
    populateSelect("assetCondition",  "computer.asset_condition",     assetConditionFallback);
	...
	function getCurrentPageKey() {
		const fileName = window.location.pathname.split("/").pop().replace(".html", "") || "assets";
	...
	function resetModal() {
		...
		document.querySelectorAll(".type-card-btn").forEach(btn => btn.classList.remove("selected"));
	...
	function updateHardwareFieldsForType(type) {
		document.querySelectorAll(".computer-only, .printer-only, .ups-only, .network-only").forEach(el => el.classList.add("hidden"));
	...
	function validateStep(step) {
		if (step === 1) {
			...
			if (assignOption.value === "Yes") {
				...
				const euNameOk = document.getElementById("euSearchCombo")?._enforceSelection?.() ?? true;
				const deptOk   = document.getElementById("euDepartmentCombo")?._enforceSelection?.() ?? true;
				const locOk    = document.getElementById("euLocationCombo")?._enforceSelection?.() ?? true;
				...
				if (contactVal !== "") {
					if (!/^09\d{9}$/.test(contactVal)) {
						setFieldError("euContactNumber", "Must be 11 digits starting with 09 (e.g. 09000000000).");
						return false;
					} else {
	...
	function populateReviewStep() {
		// Asset Basics
		document.getElementById("revComputerName").textContent = document.getElementById("computerName").value || "-";
		...
		document.getElementById("revComputerStatus").textContent = document.getElementById("computerStatus").value || "-";
		...
		document.getElementById("revOperatingSystem").textContent = document.getElementById("operatingSystem").value || "-";
		document.getElementById("revProcessor").textContent = document.getElementById("processor").value || "-";
		document.getElementById("revRamSize").textContent = document.getElementById("ramSize").value || "-";
		document.getElementById("revStorageType").textContent = document.getElementById("storageType").value || "-";
		document.getElementById("revStorageCapacity").textContent = document.getElementById("storageCapacity").value || "-";
		...
		document.getElementById("revVpnAccess").textContent = document.getElementById("hasVpnAccess").value || "-";
		...
		document.getElementById("revNetworkConnectivity").textContent = document.getElementById("networkConnectivity").value || "-";
		document.getElementById("revAnydeskIp").textContent = document.getElementById("anyDeskIp").value || "-";
		...
		if (isAssigned) {
			...
			document.getElementById("revEuContactNumber").textContent = document.getElementById("euContactNumber").value || "-";
			document.getElementById("revAssignedDate").textContent = document.getElementById("assignedDate").value || "-";
			document.getElementById("revToReturnBy").textContent = document.getElementById("toReturnBy").value || "-";
		} else {
	...
	async function handleSubmit() {
		...
		if (!confirmAdd) { _isSubmitting = false; return; }
		...
		try {
			...
			const selectedPeripherals = [...document.querySelectorAll("input[name='peripheral']:checked")]
			...
			const selectedPrograms = [...document.querySelectorAll("input[name='program']:checked")]
				.map(cb => ({ program_name: cb.value, program_id: parseInt(cb.dataset.id, 10) }));
			...
			const payload = {
				...
				networkConnectivity: document.getElementById("networkConnectivity").value,
				...
				euContactNo:     assignUserSelected ? document.getElementById("euContactNumber").value.trim() || null : null,
				assignedDate:    assignUserSelected ? document.getElementById("assignedDate").value : null,
				toReturnBy:      assignUserSelected ? (document.getElementById("toReturnBy").value || null) : null,
	...
	const DEFAULT_VISIBLE = new Set([
		"computer_name", "brand", "model", "device_type",
		"assigned_user_name", "assigned_user_emp_id", "department_name",
		"ip_address", "eu_location", "computer_status", "anyDeskIp", "action"
	]);
	...
	const FILTERABLE_COLUMNS = [
		...
		{ key: "device_type",          group: "computer.device_type",      fallback: deviceTypeFallback },
		{ key: "operating_system",     group: "computer.operating_system", fallback: osFallback },
		{ key: "ram_size",             group: "computer.ram_size",         fallback: ramFallback },
		{ key: "storage_type",         group: "computer.storage_type",     fallback: stTypeFallback },
		{ key: "storage_capacity",     group: "computer.storage_capacity", fallback: stCapFallback },
		{ key: "asset_condition",      group: "computer.asset_condition",  fallback: assetConditionFallback },
		...
		{ key: "computer_status",      options: ["Active", "Repair", "Spare", "Defective"] },
		{ key: "has_vpn_access",       options: ["Yes", "No"], transform: v => v === true ? "Yes" : v === false ? "No" : "" },
	...
	function renderTable(data) {
		...
		data.forEach(asset => {
			...
			const cells = [
				...
				{ key: "has_vpn_access",       html: asset.has_vpn_access ? "Yes" : asset.has_vpn_access === false ? "No" : "-" },
				...
				{ key: "action",               html: `<a href="computerItem.html?id=${asset.computer_id}" class="action-btn" title="Open"><i data-lucide="external-link"></i></a>` },
			];
	...
	// --- Search --------------------------------------------------
	const DATE_KEYS = ["received_date", "warranty_expiry", "assigned_date", "to_return_by"];

	// ── Column Sorting (single dropdown button, like column filters) ──
	const SORTABLE_COLUMNS = ["cost", "received_date", "warranty_expiry", "assigned_date", "to_return_by", "assigned_user_name", "computer_name", "assigned_user_emp_id"];
	const ALPHA_SORT_COLUMNS = ["assigned_user_name", "computer_name", "assigned_user_emp_id"]; // sorted alphabetically instead of numerically
